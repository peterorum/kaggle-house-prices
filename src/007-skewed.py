...
# log features on skewed columns
...
def get_log_features(train, test, target):
  ...
  skewed_features = train[numeric_cols].apply(lambda x: skew(x.dropna()))  # compute skewness
  skewed_features = skewed_features[skewed_features > 0.75]
  skewed_cols = skewed_features.index

  for col in skewed_cols:
    train[f'log_{col}'] = train[col].apply(lambda x: np.log1p(x) if x > -1 else 0)
    test[f'log_{col}'] = test[col].apply(lambda x: np.log1p(x) if x > -1 else 0)
